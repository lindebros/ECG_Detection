...
void findRPeaks(int peak){
	...
	if(peak <= THRESHOLD1){
		...
		if(RR < RR_HIGH){
			...
			if(RR_AVERAGE1 == RR_Checker){
						RR_Inc++;
					}else{
						RR_Checker = RR_AVERAGE1;
						RR_Inc = 0;
					}
		}
...
void dataOutput(int peak, int RR, int place){
	...
	if(peak < 2000 && RR_Inc >= 5){
		printf("R-Peak detected: %i, at time: %f seconds.\nThe patients pulse: %f beats per minutes. %i\n"
				"Warning: Beat to low!\nWarning: Out of rythm!\n\n", peak, time, pulse,place );
	}else if(peak < 2000){
		printf("R-Peak detected: %i, at time: %f seconds.\nThe patients pulse: %f beats per minutes. %i\n"
				"Warning: Beat to low!\n\n", peak, time, pulse,place );
	}else if(RR_Inc >= 5){
